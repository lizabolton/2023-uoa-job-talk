...
- students have had exposure to programming in R & RStudio and using the `tidyverse`,
...
- Know what a reprex is, understand when and why we need them, and have a basic idea of how to create one

In a course with a small number of overarching learnings outcomes, this topic would likely come under "Understand and implement prefessional collaboration and communication practices appropriate for data scientists"

---
class: center, middle

# 🧢 Please put your student hats  on now, I am going to ask you to interact with me as if you are students in a Zoom class.
...
* Useful **professional skill** for those working with R and other programming languages in further study and future employment. 
  - These concepts transferable to programming in any language.
  - You will continue to learn updated/new functions, if not whole new languages in your career. Learning how to ask for help is an evergreen skill.
...
---
## The philospophy of a good reprex

.large[
* Reprex code must run -- no faking it, make sure you can run it and that it demonstrates the issue)
* Others shouldn't *have* to run it -- Someone very familiar with R might be able to read the code and output and immediately understand what is going on
* Should be easy for others to copy + paste + run
]

.footnote[Source: Reprex dos and don'ts https://reprex.tidyverse.org/articles/reprex-dos-and-donts.html#package-philosophy]
...
<!---Ask to use chat to indicate if come across these --->
...
## 📖 For more dos and don'ts, read this RStudio article:  https://reprex.tidyverse.org/articles/reprex-dos-and-donts.html  
...
* Practice using reprexes when asking for help on Ed Discussion!
  * ❌ Screenshots
  * ❌ Upside down photos of computer screens taken from phones 😭
  * ✅ Code I could copy and run myself  + what your output looks like 

* If you've already installed the `tidyverse` package, `reprex` is installed will be loaded whenever we run `library(tidyverse)`.

* You usually won’t have to worry about the data side of reprexes for this course, as in almost all cases we will all be working with the same data. But! It is a key skill if you will be working with data and programming languages in the future.
...
There is a Thread on Ed Discussion called **reprex practice**. Go through the steps I demonstrated today on your own and post the result as a comment.

.question[Can you also identify and explain what is causing the error?]
